...
const AnalyseTweet = () => {
  ...
  const {displayTweet, error} = useSelector(state => state.tweetBox);
  ...
  const handleSubmit = async (statusId) => {
    ...
    if(!fetchedTweet){
      ...
      dispatch(tweetsActions.setError({message: 'The twitter api service is not available, try again later.'}));
      return;
    }
