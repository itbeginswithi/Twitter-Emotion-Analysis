...
const AnalyseTweet = () => {
  ...
  const {displayTweet, fetchingAnalysis} = useSelector(state => state.tweetBox);
  ...
  const handleSubmit = async (statusId) => {
    ...
    if(fetchedTweet?.errors){
      //! check if id exists
      const errrorMessage = fetchedTweet.errors[0].message;
      dispatch(tweetsActions.setError(errorMessage));
      return;
    }
    ...
    dispatch(tweetsActions.tweetAnalysis(analysis));
  }
