  ...
  const handleSubmit = async (statusId) => {
    ...
    try {
      ...
      const analysis = await (await fetch('/api/ibmWatson', {
        ...
        body: JSON.stringify({tweet: data[0]?.text}) //didn't use the tweet state instead of dataFor some reason using the tweet state requires clicking the search btn twice.
      })).json();
